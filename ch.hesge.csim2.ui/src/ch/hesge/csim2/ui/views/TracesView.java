...
public class TracesView extends JPanel implements ActionListener {
	...
	private void initListeners() {
		...
		// Listen to matcher combo
		matcherComboBox.addItemListener(new ItemListener() {			
			@Override
			public void itemStateChanged(ItemEvent e) {
		       if (e.getStateChange() == ItemEvent.SELECTED) {
		    	   updateLoadButtonState();
	...
	private void updateLoadButtonState() {
		...
		boolean canLoadMatching = scenarioComboBox.getSelectedItem() instanceof Scenario &&
			matcherComboBox.getSelectedItem() instanceof IMethodConceptMatcher 
			&& thresholdField.getText() != null 
			&& thresholdField.getText().trim().length() > 0;
			
		loadBtn.setEnabled(canLoadMatching);
			...
			if (scenario != null && matcher != null) {
				...
				SwingUtils.invokeLongOperation(TracesView.this, new Runnable() {
					...
					public void run() {
						...
						matchMap = appManager.getMethodMatchingMap(project, matcher, threshold);
