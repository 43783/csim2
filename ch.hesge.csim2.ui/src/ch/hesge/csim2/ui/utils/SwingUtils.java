...
public class SwingUtils {
	...
	// Private global attribute
	public static PrintWriter inputWriter;
	private static int lastConsoleCaretPosition = 0;

	/**
	 * Redirect standard output, error and input streams to the text area passed
	 * in argument
	 * 
	 * @param console
	 *        the console where streams are redirected
	 */
	public static void redirectStandardStreams(final JTextArea console) {

		// Keep references on standard streams
		final PrintStream standardOutput = System.out;

		// Create stream redirector
		PrintStream redirectedOutput = new PrintStream(new OutputStream() {

			@Override
			public void write(final int b) throws IOException {

				// Write output to GUI console area
				if (console != null) {
					SwingUtilities.invokeLater(new Runnable() {
						@Override
						public void run() {
							console.append(String.valueOf((char) b));
							lastConsoleCaretPosition = console.getCaretPosition();
						}
					});
				}
				else {
					standardOutput.print(String.valueOf((char) b));
				}
			}
		...
		PipedInputStream redirectedInput = new PipedInputStream();

		try {
			inputWriter = new PrintWriter(new PipedOutputStream(redirectedInput), true);
		...
		console.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					// Consume original CR/LF pressed
					e.consume();

					// Handle event within the EDT
					SwingUtilities.invokeLater(new Runnable() {
						@Override
						public void run() {

							try {
								// Retrieve command typed in console
								int length = console.getCaretPosition() - lastConsoleCaretPosition;
								String command = console.getText(lastConsoleCaretPosition, Math.max(0, length)).trim();

								// Before executing command, carriage return
								System.out.println();
	...
	public static Line getLine(Rectangle a, Rectangle b) {
		...
		Point sourcePoint = SwingUtils.getMidPoint(a);
		Point targetPoint = SwingUtils.getMidPoint(b);

		// Get line linking both rectangles
		Line linkLine = new Line(sourcePoint, targetPoint);

		// Calculate source/line intersection
		Point startPoint = SwingUtils.getIntersection(linkLine, a);

		// Calculate target/line intersection
		Point endPoint = SwingUtils.getIntersection(linkLine, b);
	...
	public static Line getDiagonal(Rectangle a, Rectangle b) {
		...
		Point pointA = SwingUtils.getMidPoint(a);
		Point pointB = SwingUtils.getMidPoint(b);
	...
	public static Point getIntersection(Line line, Rectangle rect) {
		...
		intersectionPoint = SwingUtils.getInsersection(line, topLine);
		...
		intersectionPoint = SwingUtils.getInsersection(line, bottomLine);
		if (intersectionPoint != null) {
			return intersectionPoint;
		}
		...
		intersectionPoint = SwingUtils.getInsersection(line, leftLine);
		if (intersectionPoint != null) {
			return intersectionPoint;
		}
		...
		intersectionPoint = SwingUtils.getInsersection(line, rightLine);
		if (intersectionPoint != null) {
			return intersectionPoint;
		}
	...
	public static ConceptLink hitLink(List<Concept> concepts, Point point, double distance) {
		...
		for (Concept concept : concepts) {
			for (ConceptLink link : concept.getLinks()) {
				...
				Line linkLine = SwingUtils.getLine(link.getSourceConcept().getBounds(), link.getTargetConcept().getBounds());

				// Check if distance between objects are enough
				if (linkLine != null && SwingUtils.getDistance(point, linkLine) < distance) {
					return link;
				}
			}
		}
